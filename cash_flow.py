...
def cash_flow_fn():
    ...
    df = df[['date', 'credit', 'customer_name']]
    df = df.groupby(['date','customer_name'])['credit'].sum().reset_index()
    ...
    future_dates['customer_name'] = 'xyz'
    future_dates['credit'] = 0
    ...
    while (i<l):
        ...
        globals()['df%s' % i] = globals()['df%s' % i].groupby(['QY','customer_name'])['credit'].sum().reset_index()
        list_cust_quart = globals()['df%s' % i]['customer_name'].unique().tolist()
        c = list(set(list_cust).difference(set(list_cust_quart)))
        xtra = {'customer_name': c}
        globals()['df%s' % i] = globals()['df%s' % i].append(pd.DataFrame(xtra))
        uni = globals()['df%s' % i]['QY'].unique()
        globals()['df%s' % i]['credit'] = globals()['df%s' % i]['credit'].fillna(0)
        globals()['df%s' % i]['QY'] = globals()['df%s' % i]['QY'].fillna(uni[0])
        globals()['df_final%s' % i] = globals()['df%s' % i][['customer_name','credit','QY']].copy() 
    ...
    i=0
    while(i<cl):
        globals()['df_cust%s' % i] = df_all[df_all['customer_name']==list_cust[i]]
        future_dates['customer_name'] = list_cust[i]
   ...
   # print('-----------------------------------------------------------------------------------------------------')
    ...
    while(i<cl):
        ...
        model=sm.tsa.statespace.SARIMAX(globals()['df_cust%s' % i]['credit'],order=(0, 1, 0),seasonal_order=(0,1,0,12))
        results=model.fit()
    ...
    df_major = globals()['df_cust%s' % 10].copy()
    ...
    df_major.to_csv('cash_forecast.csv', index = False)
    
    dtr = 'cash_forecast_.csv'
    return dtr
